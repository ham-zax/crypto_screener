...
def test_automated_tab_integration():
    ...
    try:
        ...
        response = requests.post(f"{base_url}/api/v2/fetch-projects",
        ...
        if response.status_code in [200, 202]:
            data = response.json()
            print(f"   ✅ SUCCESS: Project fetch initiated")
    ...
    try:
        ...
        if projects_response.status_code == 200:
            ...
            if projects:
                ...
                if response.status_code in [200, 404]:  # 404 is ok if no CSV data exists
                    print(f"   ✅ SUCCESS: CSV endpoint accessible")
                else:
                    print(f"   ❌ FAILED: HTTP {response.status_code}")
            else:
                print(f"   ⚠️  SKIPPED: No projects available to test CSV endpoint")
        else:
            print(f"   ⚠️  SKIPPED: Could not get projects for CSV test")
    except Exception as e:
        print(f"   ❌ ERROR: {e}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'app.js' in html_content:
                print(f"   ✅ SUCCESS: UI loads and includes app.js")
            else:
                print(f"   ⚠️  WARNING: app.js not found in HTML")
        else:
            print(f"   ❌ FAILED: HTTP {response.status_code}")
    except Exception as e:
        print(f"   ❌ ERROR: {e}")
