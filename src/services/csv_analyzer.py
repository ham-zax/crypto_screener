...
class CSVAnalyzer:
    ...
    def _score_divergence(self, price_slope: float, cvd_slope: float) -> float:
        ...
        for rule in self.rules['divergence_scores']:
            ...
            price_match = (
                (price_cond == 'positive' and price_slope > 0) or
                (price_cond == 'non_positive' and price_slope <= 0) or
                (price_cond == 'any')
            )
            
            cvd_match = (
                (cvd_cond == 'positive' and cvd_slope > 0) or
                (cvd_cond == 'non_positive' and cvd_slope <= 0) or
                (cvd_cond == 'any')
            )
    ...
    def analyze(self, csv_text: str) -> Dict[str, Any]:
        ...
        if len(df) < self.min_periods:
            return {"success": False, "error": f"Insufficient data: {len(df)} rows found, minimum is {self.min_periods}."}
        ...
        try:
            ...
            return {"success": False, "error": f"Linear regression calculation failed: {e}"}
