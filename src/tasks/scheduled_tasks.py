...
def cleanup_old_data(self, days_to_keep: int = 30):
    ...
    try:
        ...
        try:
            ...
            self.update_state(
                state='PROGRESS',
                meta={'status': 'Cleaning up old CSV data...'}
            )
            
            # Cleanup old CSV data that's no longer valid
            old_csv_query = session.query(CSVData).filter(
                CSVData.analyzed_at < cutoff_date,
                CSVData.is_valid == False
            )
            ...
            self.update_state(
                state='PROGRESS',
                meta={'status': 'Cleaning up log files...'}
            )
...
def health_check_task(self):
    ...
    try:
        ...
        self.update_state(
            state='PROGRESS',
            meta={'status': 'Checking Redis connection...'}
        )
        ...
        health_status['components']['redis'] = {
            ...
            'details': 'Redis connection successful' if redis_status else 'Redis connection failed'
        }
        
        # Check database connection
        self.update_state(
            state='PROGRESS',
            meta={'status': 'Checking database connection...'}
        )
