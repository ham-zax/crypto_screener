...
class DynamicScheduleManager:
        ...
        try:
            ...
            return {
                ...
                "active_schedules": len(
                    [s for s in schedule_list.values() if s.get("active", False)]
                ),
                "enabled_schedules": len(
                    [s for s in schedule_list.values() if s.get("enabled", False)]
                ),
            }
    ...
    def _create_schedule_object(
        self, schedule_type: str, schedule_value: Union[int, str, Dict]
    ):
        """
...
def get_timezone_aware_schedules() -> Dict[str, Any]:
    ...
    schedules = {
        ...
        "market_close_est": {
            ...
            "schedule_type": "crontab",
            "schedule_value": {"hour": 16, "minute": 0, "day_of_week": "1-5"},
            "timezone": "US/Eastern",
        },
        "asia_market_hours": {
            "description": "Asian Market Hours (9:00 AM JST)",
            "schedule_type": "crontab",
            "schedule_value": {"hour": 9, "minute": 0, "day_of_week": "1-5"},
            "timezone": "Asia/Tokyo",
        },
        "europe_market_hours": {
