...
class CoinGeckoMarket:
    ...
    def from_coingecko_response(cls, data: Dict[str, Any]) -> "CoinGeckoMarket":
        ...
        try:
            ...
            return cls(
                ...
                market_cap_change_percentage_24h=safe_float(
    ...
    def to_automated_project_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            or self.max_supply,  # Fallback to max_supply
...
class CoinGeckoCoinDetails:
    ...
    def from_coingecko_response(cls, data: Dict[str, Any]) -> "CoinGeckoCoinDetails":
        ...
        try:
            ...
            homepage = (
                homepage_list[0] if homepage_list else None
            )  # Only access index if list is not empty
...
class APIResponseValidator:
    ...
    def validate_markets_response(data: List[Dict[str, Any]]) -> List[CoinGeckoMarket]:
        ...
        for i, item in enumerate(data):
            try:
                ...
                if market.is_valid_for_scoring():
                    ...
                    logger.warning(
                        f"Market data for {market.id} failed validation: {market.get_validation_errors()}"
                    )
            except Exception as e:
        ...
        if errors:
            logger.warning(
                f"Validation errors in markets response: {len(errors)} failed out of {len(data)} items"
            )

        logger.info(
            f"Successfully validated {len(validated_markets)} markets out of {len(data)} total"
        )
    ...
    def filter_by_market_cap(
        ...
        if min_market_cap is not None:
            filtered = [
                m for m in filtered if m.market_cap and m.market_cap >= min_market_cap
            ]

        if max_market_cap is not None:
            filtered = [
                m for m in filtered if m.market_cap and m.market_cap <= max_market_cap
            ]

        logger.info(
            f"Filtered {len(filtered)} markets from {len(markets)} based on market cap criteria"
        )
        return filtered

    @staticmethod
    def filter_by_volume(
        markets: List[CoinGeckoMarket], min_volume: Optional[float] = None
    ) -> List[CoinGeckoMarket]:
        """
        ...
        filtered = [
            m for m in markets if m.total_volume and m.total_volume >= min_volume
        ]

        logger.info(
            f"Filtered {len(filtered)} markets from {len(markets)} based on volume criteria"
        )
        return filtered
