...
class CSVAnalyzer:
    ...
    def parse_and_validate_csv(
        ...
        try:
            ...
            logger.info(
                f"Parsed CSV with {len(df)} rows and columns: {list(df.columns)}"
            )
            ...
            if missing_headers:
                ...
                return (
                    None,
                    f"Missing required columns: {', '.join(missing_headers)}. Available columns: {available_cols}",
                )
            ...
            if len(df) < cls.MINIMUM_PERIODS:
                return (
                    None,
                    f"Insufficient data: {len(df)} periods found, minimum {cls.MINIMUM_PERIODS} required",
                )
            ...
            try:
                ...
                df["Volume Delta (Close)"] = pd.to_numeric(
                    df["Volume Delta (Close)"], errors="coerce"
                )
            except Exception as e:
            ...
            if duplicate_count > 0:
                ...
                df = df.drop_duplicates(subset=["time"], keep="last").reset_index(
                    drop=True
                )

            logger.info(
                f"CSV validation successful: {len(df)} periods from {df['time'].min()} to {df['time'].max()}"
            )
            return df, None
    ...
    @classmethod
    def calculate_accumulation_signal(
        cls, df: pd.DataFrame
        ...
        try:
            ...
            logger.info(
                f"Analyzing accumulation signal using {len(analysis_data)} periods"
            )
            ...
            price_slope, price_intercept, price_r, price_p_value, price_std_err = (
                linregress(x_values, close_prices)
            )
            ...
            cvd_slope, cvd_intercept, cvd_r, cvd_p_value, cvd_std_err = linregress(
                x_values, cvd_values
            )
            ...
            metadata = {
                ...
                "score_rationale": cls._get_score_rationale(
                    data_score, price_slope, cvd_slope, price_r_squared, cvd_r_squared
                ),
                "analyzed_at": datetime.utcnow().isoformat(),
            }

            logger.info(
                f"Accumulation analysis complete: Score={data_score:.1f}, "
    ...
    def _calculate_data_score(
        ...
        # Moderate Accumulation: Price positive, CVD flat/negative (less ideal but still positive)
            ...
            if (
                price_strong and abs(cvd_slope) < abs(price_slope) * 0.5
            ):  # CVD not strongly negative
    ...
    def _get_score_rationale(
        ...
        price_strength = (
            "strong"
            if price_r_squared > 0.7
            else "moderate"
            if price_r_squared > 0.5
            else "weak"
        )
        cvd_strength = (
            "strong"
            if cvd_r_squared > 0.7
            else "moderate"
            if cvd_r_squared > 0.5
            else "weak"
        )
...
class CSVFormatValidator:
    ...
    @staticmethod
    def validate_csv_format_preview(
        csv_text: str, max_rows: int = 10
    ) -> Dict[str, Any]:
        """
