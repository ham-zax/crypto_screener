...
class AutomatedScoringEngine:
    ...
    def calculate_valuation_potential(market_cap_usd: Optional[float]) -> float:
        ...
        logger.debug(
            f"Market cap ${market_cap_usd:,.0f} assigned valuation score: {score}"
        )
        return score
    ...
    def calculate_supply_risk(
        circulating_supply: Optional[float], total_supply: Optional[float]
    ) -> float:
        """
        ...
        logger.debug(
            f"Circulation ratio {circulation_ratio:.2%} assigned supply risk score: {score}"
        )
        return score
    ...
    def calculate_all_automated_scores(
        ...
        logger.info(
            f"Automated scoring completed for {market_data.id}: "
...
class ScoringValidator:
    ...
    def validate_market_data_for_scoring(market_data: CoinGeckoMarket) -> bool:
        ...
        if not market_data.market_cap or market_data.market_cap <= 0:
            logger.warning(
                f"Invalid market cap for {market_data.id}: {market_data.market_cap}"
            )
            return False
