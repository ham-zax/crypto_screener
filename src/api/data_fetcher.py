...
class DataFetchingService:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        logger.info(
            f"[DEBUG] DataFetchingService API Key provided: {api_key is not None}"
        )
        if api_key:
            logger.info(f"[DEBUG] DataFetchingService API Key length: {len(api_key)}")
            logger.info(
                f"[DEBUG] DataFetchingService API Key prefix: {api_key[:10]}..."
            )
        else:
        ...
        try:
            ...
            if task_id:
                DataFetchingService.update_task_progress(
                    task_id, 0, 100, "Fetching market data..."
                )
            ...
            if not raw_markets:
                ...
                return [], self._create_fetch_metadata(
                    start_time, 0, 0, "No data retrieved"
                )
            ...
            if task_id:
                DataFetchingService.update_task_progress(
                    task_id, 10, 100, "Validating market data..."
                )

            validated_markets = self.validator.validate_markets_response(raw_markets)

            # Apply additional filters
            filtered_markets = self._apply_additional_filters(
                validated_markets, active_filters
            )

            logger.info(
                f"After validation and filtering: {len(filtered_markets)} projects"
            )
            ...
            if task_id:
                DataFetchingService.update_task_progress(
                    task_id, 20, 100, "Processing projects..."
                )
            ...
            for batch_start in range(0, total_projects, batch_size):
                ...
                logger.info(
                    f"--- Starting batch {batch_start // batch_size + 1}/{(total_projects - 1) // batch_size + 1} ---"
                )
                ...
                logger.info(
                    f"Batch {batch_start // batch_size + 1} processed: {len(batch_processed)} projects, {len(batch_errors)} errors"
                )
                if batch_errors:
                ...
                if batch_end < total_projects:
                    logger.debug("Sleeping for 0.5s between batches")
                    time.sleep(0.5)  # Brief pause between batches

            # Final progress update
            if task_id:
                DataFetchingService.update_task_progress(
                    task_id, 95, 100, "Finalizing results..."
                )
            ...
            logger.info(
                f"Bulk fetch completed: {len(processed_projects)} projects processed "
    ...
    def fetch_single_project(
        self, coingecko_id: str, include_detailed_data: bool = True
        ...
        try:
            ...
            market_data_for_model = {
                ...
                "current_price": market_data_from_details.get("current_price", {}).get(
                    "usd"
                ),
                "market_cap": market_data_from_details.get("market_cap", {}).get("usd"),
                "circulating_supply": market_data_from_details.get(
                    "circulating_supply"
                ),
                "total_supply": market_data_from_details.get("total_supply"),
                # ... add any other required fields from the 'market_data' sub-object
            }
            target_market = CoinGeckoMarket.from_coingecko_response(
                market_data_for_model
            )
            coin_details = (
                CoinGeckoCoinDetails.from_coingecko_response(raw_details_with_market)
    ...
    def refresh_project_data(
        ...
        for coingecko_id in coingecko_ids:
            try:
                project_data = self.fetch_single_project(
                    coingecko_id, include_detailed_data=True
                )
                if project_data:
        ...
        logger.info(
            f"Refresh completed: {len(updated_projects)} successful, {len(failed_ids)} failed"
        )
    ...
    def _apply_additional_filters(
        self, markets: List[CoinGeckoMarket], filters: Dict[str, Any]
    ...
    def _process_project_batch(
        ...
        for i, market in enumerate(markets_batch):
            try:
                logger.info(
                    f"Processing project {i + 1}/{len(markets_batch)}: {market.id}"
                )
                ...
                if i > 0 and i % 10 == 0:
                    logger.debug("Sleeping for 0.1s after processing 10 projects")
                    time.sleep(0.1)
        ...
        logger.info(
            f"Batch completed: {len(processed_projects)} projects, {len(errors)} errors"
        )
    ...
    def _process_project(
        self, market: CoinGeckoMarket, coin_details: Optional[CoinGeckoCoinDetails]
    ) -> Dict[str, Any]:
        """
        ...
        scores = self.scoring_engine.calculate_all_automated_scores(
            market, coin_details
        )
    ...
    def _create_fetch_metadata(
        ...
        return {
            ...
            "success_rate": round(
                success_count / max(success_count + error_count, 1) * 100, 1
            ),
            "status": "success"
            if error_count == 0
            else "partial"
...
class ProjectIngestionManager:
    ...
    def run_full_ingestion(
        ...
        try:
            ...
            logger.info(
                f"Full ingestion completed: {len(projects)} projects in {time.time() - start_time:.1f}s"
            )

            return {"projects": projects, "ingestion_record": ingestion_record}
            ...
            return {"projects": [], "ingestion_record": error_record}
