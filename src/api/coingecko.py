...
class CoinGeckoClient:
    ...
    def __init__(self, api_key: Optional[str] = None, cache_ttl: int = 300):
        ...
        if api_key:
            ...
            if api_key.startswith("CG-"):
                ...
                self.api_tier = "demo"
                logger.info(
                    "[DEBUG] CoinGecko Demo API authentication configured (30 calls/minute)"
                )
            else:
                ...
                self.api_tier = "pro"
                logger.info(
                    "[DEBUG] CoinGecko Pro API authentication configured (500 calls/minute)"
                )
        else:
        ...
        logger.info(
            f"CoinGecko client initialized with {self.calls_per_minute} calls/minute limit"
        )
    ...
    def _enforce_rate_limit(self):
        ...
        if len(self.call_timestamps) >= self.calls_per_minute:
            ...
            wait_time = 61 - (
                now - oldest_call
            )  # Wait until oldest call is 61 seconds old
    ...
    def _make_request(self, endpoint: str, params: Optional[Dict] = None) -> Any:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                # Make request
                logger.info(
                    f"[DEBUG] Making CoinGecko API request to {endpoint} (attempt {attempt + 1})"
                )
                ...
                logger.info(
                    f"CoinGecko API response for {endpoint}: {json.dumps(data)[:1000]}"
    ...
    def get_coin_market_chart_range(
        self, coin_id: str, vs_currency: str, from_timestamp: int, to_timestamp: int
    ) -> Dict:
        """
    ...
    def get_markets_data_bulk(
        ...
        try:
            ...
            logger.info(
                f"Fetched {len(all_data)} total market entries across {page} pages"
            )
