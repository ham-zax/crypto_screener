...
LoggingManager.setup_logging(
    log_level="INFO", log_file="logs/omega_background_tasks.log"
)
...
if V2_DEPENDENCIES_AVAILABLE:
    ...
    try:
        ...
        with APP.app_context():
            ...
            auto_migrate = (
                os.getenv("AUTO_MIGRATE_ON_STARTUP", "true").lower() == "true"
            )
            ...
            if init_result["success"]:
                LOGGER.info(
                    f"✅ V2 Database initialization completed successfully in {init_result['total_time_ms']}ms."
                )
                # Detailed logging for successful initialization steps...
            else:
                LOGGER.error(
                    f"❌ Database initialization failed at step: {init_result.get('step', 'unknown')}"
                )
        ...
        LOGGER.warning(
            f"V2 backend modules not available: {e}. Running in V1 compatibility mode only."
        )
        DB = None
    except Exception as e:
        LOGGER.error(
            f"V2 backend initialization failed: {e}. Falling back to V1 compatibility mode."
        )
        DB = None
    ...
    LOGGER.info(
        "To enable V2 features, install dependencies: pip install -r requirements.txt"
    )
...
if V2_DEPENDENCIES_AVAILABLE:
    ...
    try:
        ...
        LOGGER.info(
            f"[DEBUG] COINGECKO_API_KEY loaded: {coingecko_api_key is not None}"
        )
        if coingecko_api_key:
            LOGGER.info(f"[DEBUG] COINGECKO_API_KEY length: {len(coingecko_api_key)}")
            LOGGER.info(
                f"[DEBUG] COINGECKO_API_KEY prefix: {coingecko_api_key[:10]}..."
            )
        else:
    ...
    try:
        ...
        LOGGER.info(
            "V2 CSV analysis services initialized successfully. CSV_ANALYZER is ready."
        )
    except Exception as e:
    ...
    try:
        ...
        LOGGER.warning(
            f"Task management services failed to initialize, using fallback: {e}"
        )
...
def database_health_check():
    ...
    if not V2_DEPENDENCIES_AVAILABLE or DB is None:
        return jsonify(
            {"status": "unavailable", "message": "V2 database not available"}
        ), 503
...
def get_migration_status():
    ...
    try:
        ...
        return jsonify(
            {"error": "Migration status query failed", "message": str(e)}
        ), 500
...
if V2_DEPENDENCIES_AVAILABLE and DB:
    ...
    def fetch_projects():
        ...
        return jsonify(
            {
                "message": "This endpoint is available but implementation is omitted for brevity."
            }
        )
    ...
    def refresh_single_project(project_id):
        ...
        try:
            ...
            updated_projects, failed_ids = DATA_FETCHER.refresh_project_data(
            ...
            if updated_projects:
                ...
                APP.logger.info(
                    f"[DEBUG] Updated all scores for project {project.id} in refresh_single_project"
                )
                DB.session.commit()
                return jsonify(
                    {
                ...
                return jsonify(
                    {"error": "Failed to refresh project data", "failed_id": failed_ids}
                ), 500
        except Exception as e:
            DB.session.rollback()
...
def analyze_project_csv(project_id):
    ...
    if not data or "csv_data" not in data:
        return jsonify(
            {"error": "Request body must be JSON and contain a 'csv_data' key."}
        ), 400
    ...
    if not analysis_result.get("success"):
        error_message = analysis_result.get(
            "error", "Analysis failed due to an unknown error."
        )
    ...
    try:
        ...
        if (
            project.narrative_score is not None
            and project.tokenomics_score is not None
            and project.data_score is not None
        ):
            project.omega_score = (
        ...
        DB.session.rollback()
        LOGGER.error(
            f"Database error while updating project {project_id} with data score: {e}"
        )
        return jsonify(
            {"error": "An internal server error occurred while saving the results."}
        ), 500
...
def get_task_status():
    ...
    if task_id:
        ...
        serializable_status_info = {
            k: make_serializable(v) for k, v in status_info.items()
        }
        ...
        serializable_all_statuses = {
            k: make_serializable(v) for k, v in all_statuses.items()
        }
