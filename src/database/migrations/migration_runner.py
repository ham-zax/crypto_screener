...
class MigrationRunner:
        ...
        try:
            ...
            for line in lines:
                ...
                if line.startswith("-- Migration:"):
                    ...
                    metadata["description"] = line.replace(
                        "-- Description:", ""
                    ).strip()
    ...
    def _get_migration_files(self) -> List[Path]:
        ...
        try:
            ...
            files = [
                f for f in self.migrations_path.glob("*.sql") if pattern.match(f.name)
            ]
    ...
    def _execute_sql_safely(
        self, sql: str, migration_name: str
                ...
                with connection.begin():
                    for i, statement in enumerate(statements):
                        ...
                        try:
                            ...
                            logger.debug(
                                f"Executed statement {i + 1}/{len(statements)} for {migration_name}"
                            )
                        except Exception:
                            logger.error(
                                f"Failed at statement {i + 1} in {migration_name}: {clean_statement[:100]}..."
                            )
                            raise
    ...
    def apply_migration(self, migration_file: Path) -> Dict:
        ...
        try:
            ...
            success, error = self._execute_sql_safely(
                migration_data["sql"], migration_data["name"]
            )
        ...
        try:
            ...
            if failed_migration:
                ...
                successful_migrations = [
                    m for m in applied_migrations if m["status"] == "applied"
                ]
    ...
    def rollback_migration(self, target_version: str) -> Dict:
        ...
        try:
            ...
            rollback_migrations = self.version_manager.get_versions_to_rollback(
                target_version
            )
            ...
            for migration in rollback_migrations:
                ...
                success, error = self._execute_sql_safely(
                    migration["rollback_sql"], f"Rollback {migration['version']}"
                )
                ...
                if success:
                    ...
                    failed_rollback = {"version": migration["version"], "error": error}
                    break
    ...
    def get_migration_status(self) -> Dict:
        ...
        try:
            ...
            pending_migrations = [
                {"version": f.stem, "name": f.name, "file_path": str(f)}
