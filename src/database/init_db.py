...
from .config import DatabaseConfig, get_engine, get_session
...
class DatabaseInitializer:
    ...
    def __init__(self, config: DatabaseConfig = None):
        ...
        logger.info(f"Database initializer created for {self.config.environment} environment")
    
    def _ensure_database_directory(self):
        """Ensure database directory exists for SQLite"""
        if self.config.environment == 'development' and 'sqlite' in self.config.database_url:
    ...
    def initialize_database(self, run_migrations: bool = True, 
                          seed_data: bool = False) -> Dict:
        """
        ...
        try:
            ...
            if self.engine:
                ...
                try:
                    ...
                    automated_projects = session.query(AutomatedProject)\
                                               .filter_by(data_source='automated').count()
                    projects_with_data = session.query(AutomatedProject)\
