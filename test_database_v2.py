...
class TestDatabaseInitialization(DatabaseTestBase):
    ...
    def test_database_initialization(self):
        ...
        self.assertTrue(result['success'], f"Database initialization failed: {result.get('error')}")
...
class TestDatabaseMigrations(DatabaseTestBase):
    ...
    def test_sqlite_migration_execution(self):
        ...
        migration_path = Path(__file__).parent / 'src' / 'database' / 'migrations' / 'scripts'
        ...
        if sqlite_migration.exists():
            ...
            self.assertTrue(result['success'], f"Migration failed: {result.get('error')}")
...
class TestDatabaseModels(DatabaseTestBase):
    ...
    def test_project_with_data_score(self):
        ...
        expected_omega = (project.narrative_score + project.tokenomics_score + project.data_score) / 3
    ...
    def test_csv_data_model(self):
        ...
        # Verify data
        retrieved_csv = self.session.query(self.CSVData).filter_by(project_id=project.id).first()
...
class TestDatabasePerformance(DatabaseTestBase):
    ...
    def test_large_dataset_query_performance(self):
        ...
        projects = self.session.query(self.AutomatedProject)\
    ...
    def test_aggregation_performance(self):
        ...
        projects_with_data = self.session.query(self.AutomatedProject)\
                                        .filter_by(has_data_score=True).count()
        
        query_time = time.time() - start_time
        
        self.assertGreater(total_projects, 0)
        self.assertLess(query_time, 1.0, "Aggregation queries took too long (>1 second)")
...
class DatabaseTestRunner:
    ...
    def run_all_tests(self) -> Dict:
        ...
        for suite_name, test_class in test_suites:
            ...
            if failures == 0 and errors == 0:
                ...
                logger.warning(f"⚠️  {suite_name}: {failures} failures, {errors} errors out of {tests_run} tests")
    ...
    def _print_summary(self, summary: Dict):
        ...
        for suite_name, result in summary['suite_results'].items():
            ...
            print(f"  {status} {suite_name}: {result['success_rate']:.1f}% ({result['tests_run']} tests)")
...
def main():
    ...
    parser.add_argument('--suite', choices=['connection', 'init', 'migration', 'model', 'performance'], 
